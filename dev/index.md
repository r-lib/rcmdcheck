...
on a source R package `.tar.gz` file, or on a folder containing your R
package. Supply `quiet = FALSE` if you want to omit the output. The
result of the check is returned, in a list with elements `errors`,
`warnings`, and `notes`. Each element is a character vector, and one
element of the character vectors is a single failure.

![animated screenshot of a terminal window demonstrating example usage
of the rcmdcheck
function.](https://cdn.jsdelivr.net/gh/r-lib/rcmdcheck@main/tools/rcmdcheck.svg)

### Programmatic usage

[`rcmdcheck()`](https://rcmdcheck.r-lib.org/dev/reference/rcmdcheck.md)
returns an `rcmdcheck` object, which you can query and manipulate.
...
turns the check results into a simple lists with the following
...
- `session_info`: A
  ...
  object, session information from within the check process.
...
parses check output from a file,
...
[`cran_check_flavours()`](https://rcmdcheck.r-lib.org/dev/reference/cran_check_flavours.md)
...
can compare two or more `rcmdcheck` objects.
[`compare_to_cran()`](https://rcmdcheck.r-lib.org/dev/reference/compare_to_cran.md)
compares an `rcmdcheck` object to the CRAN checks of the same package:
...
`rcmdcheck_process` is a
[`processx::process`](http://processx.r-lib.org/reference/process.md)
class, that can run `R CMD check` in the background. You can also use
this to run multiple checks concurrently.
[`processx::process`](http://processx.r-lib.org/reference/process.md)
methods can be used to poll or manipulate the check processes.
